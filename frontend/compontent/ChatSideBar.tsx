          ...
          {showAllUsers ? (
            ...
            <div className='space-y-2 overflow-y-auto h-full pb-4'>
              {chats.map((chat) => {
                ...
                // ✅ FIX: correctly determine if latest message was sent by me
                // latestMessage.sender is a userId string — compare to loggedInUser._id
                const isSentByMe =
                  latestMessage?.sender &&
                  loggedInUser?._id &&
                  latestMessage.sender === loggedInUser._id;

                // ✅ FIX: skip rendering if latestMessage has no text (empty object from backend)
                const hasMessage =
                  latestMessage && latestMessage.text && latestMessage.text.trim() !== "";
                    ...
                    <div className='flex items-center gap-3'>
                      ...
                      <div className='flex-1 min-w-0'>
                        <div className='flex items-center justify-between mb-1'>
                          ...
                          {unseenCount > 0 && (
                            <div className='bg-red-600 text-white text-xs font-bold rounded-full min-w-[1.4rem] h-5 flex items-center justify-center px-1.5'>
                              {unseenCount > 99 ? "99+" : unseenCount}
                            </div>
                          )}
                        </div>

                        {/* ✅ FIX: show sent/received indicator with correct arrow direction */}
                        {hasMessage && (
                          <div className='flex items-center gap-1.5'>
                            {isSentByMe ? (
                              // Arrow pointing right = I sent it
                              <svg
                              ...
                              // Arrow pointing left = they sent it
                              <svg
